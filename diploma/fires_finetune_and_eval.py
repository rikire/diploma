...
# --- Данные ---
DATA_PATH = 'datasets/forest_fires_prepared.csv'
...
model.add(layers.Conv1D(filters=64, kernel_size=7, strides=2, padding='same', activation='elu', input_shape=input_shape))
model.add(layers.SimpleRNN(64, activation='tanh', return_sequences=False, dropout=0.1, recurrent_dropout=0.1))
...
# --- Графики ---
PLOTS_DIR = 'logs/20250604/plots_fires'
...
plt.savefig(os.path.join(PLOTS_DIR, 'training_history_fires.png'))
plt.close()
...
plt.savefig(os.path.join(PLOTS_DIR, 'prediction_vs_actual_fires.png'))
plt.close()
...
plt.savefig(os.path.join(PLOTS_DIR, 'error_distribution_fires.png'))
plt.close()
